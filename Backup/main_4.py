...
class Function_Loss(torch.nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        elif(self.loss_type == "dice_coeff"):
            pass
        ...
        loss = function_loss(y_pred, y_true)
    ...
    def __parameter_checker(self, loss_type, problem, problem_type):
        ...
        if(loss_type != None):
            ...
            if (loss_type == "mae" or loss_type == "mse" or loss_type == "dice_coeff" or loss_type == "iou" or
                loss_type == "binary_cross_entropy_with_logistic_loss" or loss_type == "dice_coeff" or
                loss_type == "binary_cross_entropy_with_dice_loss"):

                self.loss_type = loss_type
                print("--> Function Loss: {0}".format(loss_type))
...
class Iteration():
    ...
    def train(self, batch_size = 32, n_epochs = 5, KFolds = 5,
        ...
        for fold, (train_idx, valid_idx) in enumerate(kFold.split(self.data_train)):
            ...
            for epoch in range (0, n_epochs, 1):
                ...
                time_epoch, epoch_loss_avg, epoch_loss_std, epoch_acc = self.__train_epoch(fold, epoch, train_loader, scaler = scaler)
                ...
                print("--> Epoch {0} of {1} | Time: {2}h{3}m{4:.2}s Train_Loss (Avg/Std): {5:.2}/{6:.2} Train_Acc (Avg): {7:.2}".format(
                    epoch + 1, n_epochs, int(time_epoch/(24*60)), int(time_epoch/60), time_epoch%60, epoch_loss_avg, epoch_loss_std, epoch_acc))
            ...
            fold_loss_avg = fold_loss_avg / len(train_loader)
            fold_loss_std = fold_loss_std / len(train_loader)
            fold_acc_avg = fold_acc / len(train_loader)
            
            
        

            validation_loss_avg, validation_loss_std, validation_acc = self.eval(data_test = valid_loader, print_output = False)
            print(validation_loss_avg)
            print(validation_loss_std)
            ...
            print("-> Fold {0} Train_Loss (Avg/Std): {1:.2}/{2:.2} Train_Acc (Avg/Std): {3:.2}/ Validation_Loss (Avg/Std): {4:.2}/{5:.2} Validation_Acc (Avg/Std): {6:.2}".format(
                fold + 1, fold_loss_avg, fold_loss_std, fold_acc_avg, validation_loss_avg, validation_loss_std, validation_acc))

        train_finish_time =  time.process_time() - train_start_time

        print("Model Training Time - {0}:{1}:{2}".format(int(train_finish_time/(24*60)), int (train_finish_time/60), train_finish_time%60))
    ...
    def __train_batch_mixed_precision(self, data, scaler):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss, avg_loss, std_loss = self.function_loss.forward(y_pred = y_pred.float(), y_true = masks.float())
            
            acc = self.__check_accuracy(mode = "accuracy_pixel", y_pred = y_pred, y_true = masks)
        ...
        return avg_loss.item(), std_loss.item(), acc
    ...
    def __train_epoch(self, kFold, epoch, data, scaler):
        ...
        for batch_i, data_batch in enumerate(data_tqdm, 1):
            ...
            avg_loss, std_loss, acc = self.__train_batch_mixed_precision(data_batch, scaler = scaler)

            # update tqdm loop
            data_tqdm.set_postfix(Batch = batch_i, Loss_Average = avg_loss, Loss_STD = std_loss, Accuracy = acc)
        ...
        epoch_loss_avg = epoch_loss_avg / len(data)
        epoch_loss_std = epoch_loss_std / len(data)
        epoch_acc_avg = epoch_acc / len(data)


        
        return time.process_time() - epoch_start_time, epoch_loss_avg, epoch_loss_std, epoch_acc_avg
    ...
    def eval_model(self, n_predictions = 5, new_data = None, debug_path = None):
        ...
        fig, axes = matplotlib.pyplot.subplots(3, 2, figsize = (3*3, 3*2))
    ...
    def __valid_epoch(self, data):
        ...
        with torch.no_grad():
            ...
            for batch_i, data_batch in enumerate(data, 1):
                ...
                if (self.problem_type == "supervised"):
                    ...
                    if (self.problem == "segmentation"):
                        ...
                        acc = self.__check_accuracy(mode = "accuracy_pixel", y_pred = Y_pred, y_true = Y)
                ...
                validation_loss_avg += avg_loss
                validation_loss_std += std_loss
                validation_acc += acc


        
        validation_loss_avg = validation_loss_avg / len(data)
        validation_loss_std = validation_loss_std / len(data)
        validation_acc = validation_acc / len(data)

        return validation_loss_avg, validation_loss_std, validation_acc


    def eval(self, data_test, print_output = True):
    
        if (print_output == True):
            print("Model Testing: Initialization")

        if (self.model_trained == False):
            print("ERROR -> Model Evaluation: Model Not Trained")
            return
        ...
        validation_loss_avg, validation_loss_std, validation_acc = self.__valid_epoch(data_test)

        if (print_output == True):

            print("-> Test_Loss (Avg/Std): {0:.2}/{1:.2} Test_Acc (Avg): {2}".format(validation_loss_avg, validation_loss_std, validation_acc))
