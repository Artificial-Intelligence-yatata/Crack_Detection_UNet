    ...
    def __parameter_checker(self, loss_type, problem, problem_type):
        ...
        if(loss_type != None):
            ...
            if (loss_type == "mae" or loss_type == "mse" or loss_type == "dice_coeff" or loss_type == "iou" or
                loss_type == "binary_cross_entropy" or
...
class Iteration():
    ...
    def __init__(self, problem, problem_type, debugMode = True,
        ...
        self.statistics_cross_validation = []
    ...
    def train(self, batch_size = 32, n_epochs = 5, KFolds = 5,
        ...
        for fold, (train_idx, valid_idx) in enumerate(kFold.split(self.data_train)):
            ...
            for epoch in range (0, n_epochs, 1):
                ...
                time_epoch, epoch_loss_avg, epoch_loss_std, epoch_acc = self.__train_epoch(kFold = fold, epoch = epoch, 
            ...
            fold_acc_avg = fold_acc  / n_epochs
            ...
            values = self.__cross_validation_eval(performance_method = self.performance_method, data_test = valid_loader)

            validation_loss_avg, validation_loss_std, validation_acc = values

            self.statistics_cross_validation.append([fold+1, validation_loss_avg, validation_loss_std, validation_acc])
    ...
    def __train_batch_mixed_precision(self, performance_method, data, scaler):
        ...
        with torch.cuda.amp.autocast():
            ...
            acc = self.__check_performance(mode = performance_method, y_pred = y_pred, y_true = masks)
    ...
    def __train_epoch(self, kFold, epoch, performance_method, data, scaler):
        ...
        for batch_i, data_batch in enumerate(data_tqdm, 1):
            ...
            avg_loss, std_loss, acc = self.__train_batch_mixed_precision(performance_method = performance_method,
            ...
            if (performance_method == "accuracy_pixel"):
                ...
                data_tqdm.set_postfix(Batch = batch_i, Loss_Average = avg_loss, Loss_STD = std_loss, Dice_Coeff = acc)
    ...
    def __cross_validation_eval(self, performance_method, data_test):
            ...
            for batch_i, data_batch in enumerate(data_tqdm, 1):
                ...
                if (self.problem_type == "supervised"):
                    ...
                    if (self.problem == "segmentation"):
                        ...
                        acc = self.__check_performance(mode = performance_method, y_pred = Y_pred, y_true = Y)
        ...
        validation_loss_avg = validation_loss_avg / i
        validation_loss_std = validation_loss_std / i
